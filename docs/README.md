...
<p>
  <a href="https://github.com/youcefbt-dz/MarketLab"><img src="https://img.shields.io/badge/version-3.2.0-0F172A?style=for-the-badge&labelColor=1E293B" alt="Version"/></a>
...
<div align="center">
  ...
  <p>
    <sub>Visual intelligence generated by <b>MarketLab v3.2.0</b> — Powered by local data warehouse.</sub>
  </p>
</div>
...
└──────────────────────────────────────────────────────────────┘
         │                    │                    │
         ▼                    ▼                    ▼
   core/signals.py      analysis/backtest.py   analysis/strategy_optimizer.py
   ...
   ADX/regime filter    dynamic sizing         auto-apply to signals
         │                    │
         ▼                    ▼
   core/sentiment.py    analysis/backtest_logger.py
...
```
core/stock_warehouse.py  →  data/AAPL.csv, MSFT.csv, ... (250+ symbols, ~1.8M rows)
...
```bash
python analysis/strategy_optimizer.py          # run 100 Bayesian trials
python analysis/strategy_optimizer.py --apply  # apply best_params.json to signals
```
...
```bash
# via main.py Mode 3, or directly:
python -c "from analysis.watchlist_scanner import scan_watchlist; scan_watchlist(top_n=20, min_score=4)"
```

Output per ticker: signal, score, price, RSI, ADX, ATR%, volume ratio, sentiment.
...
```bash
python core/stock_warehouse.py   # download / update all 250+ symbols
```
...
Predicts whether a backtest configuration will be "QUALITY" based on accumulated history.

- **4 models**: RandomForest, LogisticRegression, XGBoost (+ GradientBoosting fallback)
- **SMOTE** — handles class imbalance automatically
- **16 features**: Sharpe, Win Rate, Profit Factor, Max Drawdown, R-Multiple, and more
- Trains once, cached in memory — no repeated training per run
...
python analysis/strategy_optimizer.py --apply
```
...
optuna>=3.0.0          scikit-learn>=1.3.0    imbalanced-learn>=0.11.0
xgboost>=2.0.0         rich>=13.0.0
```
...
- [x] ML quality predictor (XGBoost + SMOTE)
- [x] Modular project structure (core / analysis / ui)
...
See [SYSTEM_RELEASE_HISTORY.md](./docs/SYSTEM_RELEASE_HISTORY.md) for full version history.

**v3.2.0** — Modular Restructure
- Reorganized into `core/`, `analysis/`, `ui/`, `assets/`, `config/` packages
- Added `analysis/watchlist_scanner.py` as standalone module
- Fixed sentiment label consistency (`Positive/Negative/Neutral`)
- Updated all cross-module imports

**v3.1.0** — Signal Filters + CLI Refactor
- Refactored `main.py` into 5-mode interactive CLI
- Added `MIN_ADX_ENTRY = 18` filter
- Added bear market deep penalty (−2)
- ATR stop loss multiplier: 1.5 → 2.5
