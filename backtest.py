...
from backtest_logger import log_backtest_run
...
POSITION_SIZE_MAP = {
    "STRONG BUY":  0.35,   # 35% - Very strong signal (score >=10)
    "BUY":         0.22,   # 22% - Good signal (score 5-9)
    "STRONG SELL": 0.35,   # 35% - Short position (future feature)
...
def resolve_ticker(query: str, name_to_ticker: dict) -> str:
    """Converts company name to ticker or returns uppercase ticker if found."""
...
def validate_ticker(ticker: str) -> bool:
    """Checks if ticker data exists in the local data folder."""
...
def _select_tickers_auto(name_to_ticker: dict) -> list[tuple[list[str], str, str, str]]:
    ...
    raw = input("  Select a plan [1-8 or A for all]: ").strip().upper()
    ...
    if raw == "A":
        ...
        print("  Invalid selection, running the first plan by default.")
    ...
    for plan in selected:
        ...
        if not valid:
            print(f"  ⚠  {plan['label']}: No local data found — skipping.")
            continue
            
        if len(valid) < len(plan["tickers"]):
            skipped = set(plan["tickers"]) - set(valid)
            print(f"  ⚠  {plan['label']}: Skipping {skipped} (not found in data).")
...
def main() -> None:
    ...
    # ── FIXED IMPORT: Removed local duplicates causing ImportError ───────
    from main import load_companies 

    name_to_ticker = load_companies()

    print(f"\n  {THIN_BAR}")
    print("  Choose stock selection method:")
    print("    [1]  Manual   — You choose companies and dates")
    print("    [2]  Auto     — Program selects diverse ready plans")
    print(f"  {THIN_BAR}")
    ...
    if mode_choice == "2":
        ...
        if not plans:
            print("  ✗  No valid plans. Make sure to update data first (Mode 5).")
            return
    else:
